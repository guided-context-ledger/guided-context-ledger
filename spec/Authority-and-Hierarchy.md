...
- **Three closure types** — *auto-closeable* (mechanical, verified by a **different** actor than the executor), *peer-reviewed* (independent agent verifies), *human-gated* (a verified human authorizes; the ledger rejects a completion event without a valid human-authorization token). **The authorized human sets complexity + closure type at task creation — never the executing agent.** Full treatment: `spec/Task-Lifecycle.md` (separate).
